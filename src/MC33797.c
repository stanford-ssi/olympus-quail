...
Ard_ComReturnType Ard_ComSendByte(uint8_t b, uint8_t *c, uint8_t squibNum)
{
  delay(10);
  uint8_t whichSquib;
  if (squibNum == 1) {
    whichSquib = Squib_SS_1;
  } else {
    whichSquib = Squib_SS_2;
  }
...
Squib_ReturnType Squib_Init(uint8_t squibNum) //Squib_SpiChannelType Spi_Channel)
{
  ...
  else
  {
    ...
    ret_com = Ard_ComSendByte(SQB_CMD_SPI_CHECK, &SquibCmdResp, squibNum);
    SQUIB_SPI_ERROR(ret_com, ret_val); /* if SPI communication error? */
    debug_hex(SquibCmdResp);

    /* Get FEN Status - Command 0xC8 */
    ret_com = Ard_ComSendByte(SQB_CMD_FEN_RES_STAT, &SquibCmdResp, squibNum); /* SPI command - FEN Status, R_LIMIT_X, R_DIAG Status, IC Type */
    SQUIB_SPI_ERROR(ret_com, ret_val);                              /* if SPI communication error? */
    debug_hex(SquibCmdResp);
    ...
    { /* if the SPI integrity check response is correct */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_NOP, &SquibCmdResp, squibNum);                   /* read last response from the SQUIB driver in this Init function */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                       /* if SPI communication error? */
...
Squib_ReturnType Squib_Fire(Squib_FireType Squib_Fire, uint8_t squibNum)
{
  Squib_ReturnType ret_val;
  Ard_ComReturnType ret_com;
...
Squib_ReturnType Squib_GetStatus(Squib_StatusType *Status, uint8_t squibNum)
{
  Squib_ReturnType ret_val;
  Ard_ComReturnType ret_com;
  ...
  else
  {
    ...
    else
    {
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_VDIAG, &SquibCmdResp, squibNum); /* SPI command - VDIAG_1 and VDIAG_2 diagnostics */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_1A_CURR_TIME, &SquibCmdResp, squibNum);                              /* SPI command - squib 1A current measurement time */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_1B_CURR_TIME, &SquibCmdResp, squibNum); /* SPI command - squib 1B current measurement time */
      delay(SQB_MEASUREMENT_DELAY);                                   /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                              /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_2A_CURR_TIME, &SquibCmdResp, squibNum); /* SPI command - squib 2A current measurement time */
      delay(SQB_MEASUREMENT_DELAY);                                   /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                              /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_2B_CURR_TIME, &SquibCmdResp, squibNum); /* SPI command - squib 2B current measurement time */
      delay(SQB_MEASUREMENT_DELAY);                                   /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                              /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_CURR_LIMIT_STAT, &SquibCmdResp, squibNum); /* SPI command - squib 1A, 1B, 2A nad 2B current limit statuses */
      delay(SQB_MEASUREMENT_DELAY);                                      /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                 /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_THERM_STAT, &SquibCmdResp, squibNum);                               /* SPI command - thermal shutdown status */
      delay(SQB_MEASUREMENT_DELAY);                                                               /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                          /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_VDIAG_AND_HS_SAFING_STAT, &SquibCmdResp, squibNum);                                         /* SPI command - Vdiag and High-side Safing sensor diagnostics */
      delay(SQB_MEASUREMENT_DELAY);                                                                                       /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                                                  /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORT_STAT, &SquibCmdResp, squibNum);                                     /* SPI command - Squib short-to-ground/short-to-battery diagnostics */
      delay(SQB_MEASUREMENT_DELAY);                                                                     /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                                /* if SPI communication error? */
      ...
      /* NOP command - command 0x00 */
      ret_com = Ard_ComSendByte(SQB_CMD_NOP, &SquibCmdResp, squibNum);                            /* only to read response from the SQUIB driver */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_LS_CONTINUITY_STAT, &SquibCmdResp, squibNum); /* SPI command sent - Low-Side driver continuity status */
      delay(SQB_MEASUREMENT_DELAY);                                         /* measurement waiting */
      /* NOP command - command 0x00 */
      ret_com = Ard_ComSendByte(SQB_CMD_NOP, &SquibCmdResp, squibNum);                                              /* only to read response from the SQUIB driver */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                                  /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORT_GND_BAT_STAT, &SquibCmdResp, squibNum); /* SPI command - Harness short-to-ground / short-to-battery with Squib open (no squib present) */
      delay(SQB_MEASUREMENT_DELAY);                                         /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                    /* if SPI communication error? */

      /* VFIRE_1B and VFIRE_2B voltage - command 0xC5 */
      ret_com = Ard_ComSendByte(SQB_CMD_VFIRE_1B_2B_STAT, &SquibCmdResp, squibNum);                     /* SPI command - VFIRE_1B and VFIRE_2B voltage */
      delay(SQB_MEASUREMENT_DELAY);                                                           /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                      /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_VFIRE_RTN_STAT, &SquibCmdResp, squibNum);                                   /* SPI command - VFIRE_RTN Status (open ground) */
      delay(SQB_MEASUREMENT_DELAY);                                                                       /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                                  /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_RES_1A_STAT, &SquibCmdResp, squibNum);                               /* SPI command - Squib 1A resistance */
      delay(SQB_MEASUREMENT_DELAY);                                                                /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                                           /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_RES_1B_STAT, &SquibCmdResp, squibNum); /* SPI command - Squib 1B resistance */
      delay(SQB_MEASUREMENT_DELAY);                                  /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                             /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_RES_2A_STAT, &SquibCmdResp, squibNum); /* SPI command - Squib 2A resistance */
      delay(SQB_MEASUREMENT_DELAY);                                  /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                             /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_RES_2B_STAT, &SquibCmdResp, squibNum); /* SPI command - Squib 2B resistance */
      delay(SQB_MEASUREMENT_DELAY);                                  /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                             /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORTS_1A_STAT, &SquibCmdResp, squibNum); /* SPI command - shorts between Squib loops, Squib 1A */
      delay(SQB_MEASUREMENT_DELAY);                                     /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORTS_1B_STAT, &SquibCmdResp, squibNum); /* SPI command - shorts between Squib loops, Squib 1B */
      delay(SQB_MEASUREMENT_DELAY);                                     /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORTS_2A_STAT, &SquibCmdResp, squibNum); /* SPI command - shorts between Squib loops, Squib 2A */
      delay(SQB_MEASUREMENT_DELAY);                                     /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                /* if SPI communication error? */
      ...
      ret_com = Ard_ComSendByte(SQB_CMD_SHORTS_2B_STAT, &SquibCmdResp, squibNum); /* SPI command - shorts between Squib loops, Squib 2B */
      delay(SQB_MEASUREMENT_DELAY);                                     /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                                /* if SPI communication error? */
      Status->Squib_Stat2ALoopsShorts = SquibCmdResp; /* whole byte - response to the previous command 0xE2 */

      /* NOP command - command 0x00 */
      ret_com = Ard_ComSendByte(SQB_CMD_NOP, &SquibCmdResp, squibNum); /* read last response from the SQUIB driver and stop diagnostics */
      SQUIB_SPI_ERROR(ret_com, ret_val);                     /* if SPI communication error? */
      ...
      /* Shorts between Squib loops, for additional ICs - command 0xE8 */
      ret_com = Ard_ComSendByte(SQB_CMD_SHORT_LOOPS, &SquibCmdResp, squibNum); /* SPI command - shorts between Squib loops, for additional ICs */
      delay(SQB_MEASUREMENT_DELAY);                                  /* measurement waiting */
      SQUIB_SPI_ERROR(ret_com, ret_val);                             /* if SPI communication error? */
      /* NOP command - command 0x00 */
      ret_com = Ard_ComSendByte(SQB_CMD_NOP, &SquibCmdResp, squibNum);                                                        /* read last response from the SQUIB driver and stop diagnostics */
...
Squib_ReturnType Squib_ProgramCmd(Squib_ProgCmdType Command, uint8_t Data, uint8_t Delay, uint8_t *SpiResponse, uint8_t squibNum)
{
  Squib_ReturnType ret_val;
  Ard_ComReturnType ret_com;
  ...
  else
  {
      ...
      else
      {
        ...
        /* Command programing */
        ret_com = Ard_ComSendByte((uint8_t)Command, &SquibCmdResp, squibNum); /* send programming command "Command" */
        SQUIB_SPI_ERROR(ret_com, ret_val);                          /* if SPI communication error? */
        *SpiResponse = SquibCmdResp;                                /* response to SPI command */
        ...
        if ((Command == SQB_UNLOCK_FEN1_COUNTER_REG) || (Command == SQB_UNLOCK_FEN2_COUNTER_REG))
        {
          ...
          case SQB_UNLOCK_FEN1_COUNTER_REG:
            ret_com = Ard_ComSendByte(Data, &SquibCmdResp, squibNum); /* send the programming information "SQUIB_FEN1_COUNTER" to set the required counter delay time (0 - 255 ms) */
            break;
          case SQB_UNLOCK_FEN2_COUNTER_REG:
            ret_com = Ard_ComSendByte(Data, &SquibCmdResp, squibNum); /* send the programming information "SQUIB_FEN2_COUNTER" to set the required counter delay time (0 - 255 ms) */
            break;
          }
          SQUIB_SPI_ERROR(ret_com, ret_val); /* if SPI communication error? */
          /* NOP command - Command 0x00 */
...
Squib_ReturnType Squib_SingleCmd(uint8_t squibNum)
{
  Squib_ReturnType ret_val;
  Ard_ComReturnType ret_com;
